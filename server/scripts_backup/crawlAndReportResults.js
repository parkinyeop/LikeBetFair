...
(async () => {
      ...
      $('td').filter((i, el) => $(el).text().trim().match(/^\d{1,2} [A-Z][a-z]{2}$/)).each((i, el) => {
        ...
        if (tds.length >= 4) {
          ...
          if (isoDate >= minDate && isoDate <= today) {
            ...
            let score = $(tds[2]).text().trim();
            const away = $(tds[3]).text().trim();
            // 스코어가 '숫자 - 숫자' 형식일 때만 인정, 아니면 빈 문자열로
            if (!score.match(/^\d+\s*-\s*\d+$/)) {
              if (score.length > 0) {
                console.warn(`[${league}] ${isoDate} ${home} vs ${away} - 비정상 score: '${score}'`);
              }
              score = '';
            }
  ...
  // 크롤링 결과를 파일로 저장
  fs.writeFileSync('allResults.json', JSON.stringify(allResults, null, 2), 'utf-8');
})(); 