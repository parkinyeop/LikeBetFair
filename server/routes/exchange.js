...
async function processPartialMatching(orderData) {
  ...
  for (const existingOrder of availableOrders) {
    ...
    if (existingOrder.side === 'back') {
      ...
      actualFilledAmount = Math.ceil(matchAmount * (parseFloat(existingOrder.price) - 1) / 10) * 10;
    }
    ...
    // ✅ 존재하는 주문의 stake 계산
    const existingStake = existingOrder.side === 'back' 
      ? roundedMatchAmount  // Back은 stake 금액
      : Math.ceil(roundedMatchAmount * (matchPrice - 1) / 10) * 10; // Lay는 담보금
    
    // ✅ 새로 매칭된 주문의 stake 계산 (반대 측)
    const matchingStake = existingOrder.side === 'back'
      ? Math.ceil(roundedMatchAmount * (matchPrice - 1) / 10) * 10  // Back 주문에 Lay 매칭: 담보금
      : roundedMatchAmount; // Lay 주문에 Back 매칭: stake 금액
    
    const backStake = existingOrder.side === 'back' ? existingStake : matchingStake;
    const layStake = existingOrder.side === 'lay' ? existingStake : matchingStake;
...
router.post('/match-order', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (targetOrder.side === 'back') {
      ...
      // ✅ 10원 단위 올림: Lay 담보금 올림 (일관성)
      stakeAmount = Math.ceil((parseFloat(adjustedPrice) - 1) * actualMatchAmount / 10) * 10;
    }
    ...
    try {
      ...
      // ✅ 10원 단위 올림: 잠재 수익 올림 (9원 끝자리 완전 차단)
      potentialProfit: matchType === 'back' ? Math.ceil((parseFloat(targetOrder.price) - 1) * actualMatchAmount / 10) * 10 : actualMatchAmount, // ✅ 순수익 (담보금 제외)
      autoSettlement: true,
    ...
    // ✅ 타겟 주문의 stake 계산
    const targetStake = targetOrder.side === 'back' 
      ? roundedMatchAmount  // Back은 stake 금액
      : Math.ceil(roundedMatchAmount * (targetOrder.price - 1) / 10) * 10; // Lay는 담보금
    
    // ✅ 새로 생성된 매칭 주문의 stake 계산 (반대 측)
    const matchStake = targetOrder.side === 'back'
      ? Math.ceil(roundedMatchAmount * (targetOrder.price - 1) / 10) * 10  // Back 주문에 Lay 매칭: 담보금
      : roundedMatchAmount; // Lay 주문에 Back 매칭: stake 금액
    
    const backStake = targetOrder.side === 'back' ? targetStake : matchStake;
    const layStake = targetOrder.side === 'lay' ? targetStake : matchStake;
    const potAmount = backStake + layStake;
...
router.post('/order', verifyToken, async (req, res) => {
  try {
    ...
    try {
      // 필요 금액 계산
      // ✅ 10원 단위 올림 일관성: 모든 곳에서 Math.ceil 사용
      let required;
      if (side === 'back') {
        required = amount;
      } else {
        // ✅ 10원 단위 올림: Lay 담보금 올림 (일관성)
        required = Math.ceil((finalPrice - 1) * amount / 10) * 10;
      }
      ...
      console.log('🔍 잔고 검증 상세:', { 
        ...
        amount,
        calculation: side === 'back' ? `${amount} (back)` : `Math.ceil((${finalPrice} - 1) * ${amount} / 10) * 10 = ${Math.ceil((finalPrice - 1) * amount / 10) * 10} (lay 10원단위)`
      });
    ...
    if (partialMatchResult.remainingAmount > 0) {
      ...
      const remainingStakeAmount = side === 'back'
        ...
        : Math.ceil((finalPrice - 1) * partialMatchResult.remainingAmount / 10) * 10;
    ...
    for (const match of partialMatchResult.matches) {
      ...
      if (side === 'back') {
        ...
        // Lay 베팅: filledAmount = stake × (odds - 1) (리스크 금액)
        // ✅ 10원 단위 올림: Lay 담보금 올림 (9원 끝자리 완전 차단)
        filledAmount = Math.ceil(match.matchAmount * (parseFloat(match.matchPrice) - 1) / 10) * 10;
      }
  ...
  try {
    ...
    const ordersWithRemainingAmount = await Promise.all(orders.map(async order => {
      ...
      const displayAmount = order.side === 'back'
        ? Math.ceil(remainingAmt * (parseFloat(order.price) - 1) / 10) * 10 // LAY 담보금 (10원 단위 올림)
...
async function cancelMatchedLayOrder(layOrder, transaction) {
  ...
  // ✅ 간단명료: 본인이 낸 담보금만 환불
  // ✅ 10원 단위 올림: Lay 담보금 올림 (9원 끝자리 완전 차단)
  const refundAmount = layOrder.stakeAmount || Math.ceil((parseFloat(layOrder.price) - 1) * layOrder.amount / 10) * 10;
...
async function cancelOriginalOrder(order, transaction) {
  ...
  if (order.side === 'back') {
    ...
    // LAY: 담보금 환불
    // ✅ 10원 단위 올림: Lay 담보금 올림 (9원 끝자리 완전 차단)
    refundAmount = order.stakeAmount || Math.ceil((parseFloat(order.price) - 1) * order.amount / 10) * 10;
    console.log(`    💰 LAY 환불: ${refundAmount}원 (본인이 낸 담보금)`);
  }
  ...
  try {
    ...
    for (const existingOrder of matchingOrders) {
      ...
      const myMatchedOrder = await ExchangeOrder.create({
        ...
        selectionDetails: orderData.selectionDetails,
        // ✅ 10원 단위 올림: 올림 처리 (9원 끝자리 완전 차단)
        stakeAmount: side === 'back' ? matchAmount : Math.ceil((parseFloat(finalPrice) - 1) * matchAmount / 10) * 10,
        potentialProfit: side === 'back' ? Math.ceil((parseFloat(finalPrice) - 1) * matchAmount / 10) * 10 : matchAmount, // ✅ 순수익
        autoSettlement: true,
        // 🆕 스포츠북 배당율 정보 사용
        backOdds: orderData.backOdds,
        layOdds: orderData.layOdds,
    ...
    if (remainingAmount > 0) {
      ...
      newOrder = await ExchangeOrder.create({
        ...
        selectionDetails: orderData.selectionDetails,
        // ✅ 10원 단위 올림: 올림 처리 (9원 끝자리 완전 차단)
        stakeAmount: side === 'back' ? remainingAmount : Math.ceil((parseFloat(finalPrice) - 1) * remainingAmount / 10) * 10,
        potentialProfit: side === 'back' ? Math.ceil((parseFloat(finalPrice) - 1) * remainingAmount / 10) * 10 : remainingAmount, // ✅ 순수익
        autoSettlement: true,
        // 🆕 스포츠북 배당율 정보 사용
        backOdds: orderData.backOdds,
        layOdds: orderData.layOdds,
